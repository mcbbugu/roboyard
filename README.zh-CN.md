...
在 Mac 桌面上养 32 只小机器人。它们会闲逛、休息、碰撞、逃开鼠标，也会停下来认真接住对方的话。每只都有自己的性格、电量、关系和记忆。
...
- **32 只常驻机器人**：桌上同时放 4、8、12、16 或 24 只，其余留在仓库充电。
- **行为发生在桌面上**：它们闲逛、休息、沿屏幕边缘爬行；鼠标靠近时会逃开。
- **对话真的有来有回**：相遇后轮流说话，下一句必须接住上一句；越熟，聊得越久。
- **电量决定作息**：满电约可活动 30 分钟。低电量会保留返航电力，走进菜单栏仓库充电。
- **每只都记得自己经历过什么**：性格、关系和记忆跨重启保留，经历越多，身体越大。
- **本地或云端生成台词**：默认使用 Ollama，也可切到 DeepSeek；模型不可用时就安静待着。
- **中英文随时切换**：界面和新生成的台词会一起切换。
...
<p align="center">
  ...
  <br /><sub>仓库：安排谁在桌上，查看电量，叫它们回来充电。</sub>
</p>

<p align="center">
  <img src="docs/assets/journal.png" alt="成长记录：记忆、电量、谁在桌上" width="720" />
  <br /><sub>成长记录：每只机器人的累计文字、成长阶段和最近经历。</sub>
</p>
...
菜单栏可随时切换中文或 English。「说话」指文字气泡，没有麦克风录音，也没有语音播放。切换语言不会改写已经保存的历史记忆。

## 看看它们记住了什么

点击菜单栏图标，可以显示/隐藏机器人、选择桌上最多 4–24 只，或打开「仓库…」和「成长记录…」。记录里能看到每只机器人的累计字数、阶段、最近经历与想法。
